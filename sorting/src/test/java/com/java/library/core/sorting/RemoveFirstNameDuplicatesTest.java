...
public class RemoveFirstNameDuplicatesTest {
	...
	public void eliminateDuplicates1() throws Exception {
		...
		A = Arrays.asList(new RemoveFirstNameDuplicates.Name("Ian", "Botham"),
				new RemoveFirstNameDuplicates.Name("David", "Gower"), new RemoveFirstNameDuplicates.Name("Ian", "Bell"),
	...
	private void test(List<String> expected, List<RemoveFirstNameDuplicates.Name> A) {
		...
		List<String> result = A.stream().map(name -> name.first).collect(Collectors.toList());
